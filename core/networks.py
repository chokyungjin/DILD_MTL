...
class DILD_LSTM(nn.Module): 
    def __init__(self,num_classes,layer):
        super().__init__()
        
        self.model = convLSTM.ConvLSTM(512, 256, (3,3), layer, True, True, False)
        self.GAP = nn.AdaptiveAvgPool2d((1,1))
        self.conv2d = nn.Conv2d(256,num_classes,(1,1),1,0)

    def forward(self,x):
        
        
        _,last_states = self.model(x)

        x = self.conv2d(last_states[0][0])
        gap_1 = self.GAP(x)
        logits = gap_1[:,:,0,0]
